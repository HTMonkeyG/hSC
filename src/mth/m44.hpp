...
struct m44 {
  ...
  // Construct a matrix with given rows.
  inline m44(
    const v4f &row1,
    const v4f &row2,
    const v4f &row3,
  ...
  inline m44rows *operator->() {
    return &r;
  }

  inline const m44rows *operator->() const {
    return &r;
  }

  // Add two matrices element by element.
  inline friend m44 operator+(
    const m44 &a,
    const m44 &b
  ) {
    return m44{
  ...
  // Substract two matrices element by element.
  inline friend m44 operator-(
    const m44 &a,
    const m44 &b
  ) {
  ...
  // We don't allow a matrix to be added or substracted by a number.
  friend m44 operator+(
    const m44 &, const f32 &) = delete;
  friend m44 operator+(
    const f32 &, const m44 &) = delete;
  friend m44 operator-(
    const m44 &, const f32 &) = delete;
  friend m44 operator-(
    const f32 &, const m44 &) = delete;
  ...
  // Produce matrix number multiplication.
  inline friend m44 operator*(
    const f32 &a,
    const m44 &b
  ) {
  ...
  // Transpose the matrix.
  inline m44 transpose() const {
    m44 t;
    (void)m44transpose(&t, this);
    return t;
  }
};
...
static inline m44 *m44mul(
  ...
  r->m128[0] = _mm256_castps256_ps128(c0);
  r->m128[1] = _mm256_extractf128_ps(c0, 1);
  r->m128[2] = _mm256_castps256_ps128(c1);
...
static inline m44 *m44transpose(
  ...
  // b0 = [a00, a20, a01, a21, a10, a30, a11, a31]
  // b1 = [a02, a22, a03, a23, a12, a32, a13, a33]
  b0 = _mm256_unpacklo_ps(r01, r23);
  b1 = _mm256_unpackhi_ps(r01, r23);

  // c0 = [a00, a20, a01, a21, a02, a22, a03, a23]
  // c1 = [a10, a30, a11, a31, a12, a32, a13, a33]
  ...
  // t2 = [a10, a30, a11, a31]
  // t3 = [a12, a32, a13, a33]
  t0 = _mm256_extractf128_ps(c0, 0);
  t1 = _mm256_extractf128_ps(c0, 1);
  t2 = _mm256_extractf128_ps(c1, 0);
  t3 = _mm256_extractf128_ps(c1, 1);

  // [a00, a10, a20, a30]
  // [a01, a11, a21, a31]
  // [a02, a12, a22, a32]
  // [a03, a13, a23, a33]
  r->m128[0] = _mm_unpacklo_ps(t0, t2);
  r->m128[1] = _mm_unpackhi_ps(t0, t2);
  r->m128[2] = _mm_unpacklo_ps(t1, t3);
  r->m128[3] = _mm_unpackhi_ps(t1, t3);
